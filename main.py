...
def setup_material(context, json_file_path, texture_directories):
    ...
    textures = {
        key: value.split("/")[-1].split(".")[0] for key, value in textures.items()
    }

    material_name = os.path.splitext(os.path.basename(json_file_path))[0]

    print(f"Materi Name: {material_name}")
    ...
    if material:
        ...
        material.node_tree.links.new(
            principled_node.outputs[0], material_output_node.inputs[0]
        )
        ...
        for texture_type, texture_name in textures.items():
            ...
            found = False
            print("Texture Directory: ", texture_directories)
            for texture_directory in texture_directories.split(";"):
                abs_texture_path, found_texture_type = find_textures(
                    texture_directory, {texture_type: texture_name}
                )
                ...
                if abs_texture_path:
                    ...
                    try:
                        ...
                        if found_texture_type == "BaseColor":
                            material.node_tree.links.new(
                                texture_node.outputs[0], mix_rgb_node.inputs[1]
                            )
                            material.node_tree.links.new(
                                mix_rgb_node.outputs[0], mix_rgb_node2.inputs[1]
                            )
                            material.node_tree.links.new(
                                separate_rgb_node.outputs[0], mix_rgb_node2.inputs[2]
                            )
                            material.node_tree.links.new(
                                mix_rgb_node2.outputs[0], principled_node.inputs[0]
                            )
                        elif found_texture_type == "SSS":
                            material.node_tree.links.new(
                                texture_node.outputs[0], principled_node.inputs[3]
                            )
                        elif found_texture_type == "ORM":
                            material.node_tree.links.new(
                                texture_node.outputs[0], separate_rgb_node.inputs[0]
                            )
                            material.node_tree.links.new(
                                texture_node.outputs[1], invert_node.inputs[1]
                            )
                            material.node_tree.links.new(
                                invert_node.outputs[0], principled_node.inputs[7]
                            )
                            material.node_tree.links.new(
                                separate_rgb_node.outputs[1], principled_node.inputs[9]
                            )
                            material.node_tree.links.new(
                                separate_rgb_node.outputs[2], principled_node.inputs[6]
                            )
                        elif found_texture_type == "Normal":
                            material.node_tree.links.new(
                                texture_node.outputs[0], normal_converter.inputs[0]
                            )
                            material.node_tree.links.new(
                                normal_converter.outputs[0], normal_map_node.inputs[1]
                            )
                            normal_converter.inputs[1].default_value = (
                                opengl_directx_flip
                            )
                            material.node_tree.links.new(
                                normal_map_node.outputs[0], principled_node.inputs[22]
                            )
                        elif found_texture_type == "Mask":
                            material.node_tree.links.new(
                                texture_node.outputs[0], separate_rgb_node3.inputs[0]
                            )
                            material.node_tree.links.new(
                                texture_node.outputs[0], mix_rgb_node.inputs[2]
                            )
                            material.node_tree.links.new(
                                separate_rgb_node3.outputs[0], mix_rgb_node.inputs[0]
                            )
                        else:
                            print(f"Unhandled texture type: {found_texture_type}")

                        print(
                            "Number of links in material:",
                            len(material.node_tree.links),
                        )
            ...
            if not found:
                print(
                    f"No matching texture found for material '{material_name}' and type '{texture_type}'."
                )
...
def register():
    ...
    bpy.types.Scene.json_file_path = bpy.props.StringProperty(
        name="JSON File Path", subtype="FILE_PATH"
    )
    bpy.types.Scene.texture_directories = bpy.props.StringProperty(
        name="Texture Directories", subtype="DIR_PATH"
    )
